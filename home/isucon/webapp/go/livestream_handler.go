...
func reserveLivestreamHandler(c echo.Context) error {
	...
	livestream, err := txFillLivestreamResponse(ctx, tx, *livestreamModel)
	if err != nil {
...
func fillLivestreamResponse(ctx context.Context, livestreamModel LivestreamModel) (Livestream, error) {
	...
	for _, livestreamTagModel := range livestreamTagModels {
		tagIds = append(tagIds, int(livestreamTagModel.TagID))
	}
...
func txFillLivestreamResponse(ctx context.Context, tx *sqlx.Tx, livestreamModel LivestreamModel) (Livestream, error) {
	ownerModel := UserModel{}
